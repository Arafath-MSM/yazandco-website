(function () {
	const slideshow = () => {
		document.querySelectorAll(".slideshow").forEach((block) => {
			...
			const hiddenSlideSelector = window.matchMedia("(max-width: 575px)").matches
				? ".slideshow__content--desktop-only"
				: ".slideshow__content--mobile-only";

			block[0].querySelectorAll(hiddenSlideSelector).forEach((slide) => {
				const tab = slide.dataset.tab;
				if (tab && swiperTextElement) {
					swiperTextElement
						.querySelectorAll(`.slideshow__tab[data-anchor="${tab}"]`)
						.forEach((tabElement) => tabElement.remove());
				}
				slide.remove();
			});
